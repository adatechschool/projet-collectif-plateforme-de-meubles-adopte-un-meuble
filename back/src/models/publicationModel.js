...
const getPublicationById = async (req, res) => {
    const { data, error } = await supabase.from("Publications").select("Utilisateur!inner(pseudo),Type!inner(type), description, date, Statut_Publication!inner(statut), titre, prix, photos, Couleur!inner(couleur), Matière!inner(matière), État_Meuble!inner(état), Dimensions!inner(hauteur,largeur,longueur), Pièce!inner(pièce),id, en_valeur").eq("id", req.params.id);
    if (error) throw error;
    return data;
};
...
const getEssentials = async (req, res) => {
    const { data, error } = await supabase.from("Publications").select("id, titre, photos, prix, État_Meuble!inner(état), en_valeur").eq("en_valeur", true);
    if (error) throw error;
    return data;
};
...
const getPanier = async (req, res) => {
  const { data, error } = await supabase
    .from("Panier")
    .select(
      "id,date,Statut_Publication!inner(statut), Utilisateur!public_Panier_acheteur_id_fkey(pseudo), Publications!inner(prix,titre)"
    )
...
const addCart = async (req, res) => {
    try {
        ...
        console.error("Erreur lors de l'ajout de l'article au panier:", error.message);
