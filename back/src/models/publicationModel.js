...
const getPublicationById = async (req, res) => {
    const { data, error } = await supabase.from("Publications").select("User!inner(pseudo),Type!inner(type), description, date, Statut_Publication!inner(statut), titre, prix, photos, Couleur!inner(couleur), Matière!inner(matière), État_Meuble!inner(état), Dimensions!inner(hauteur,largeur,longueur), Pièce!inner(pièce),id").eq("id", req.params.id);
    if (error) throw error;
    return data;
};
...
const filterPublication = async (req, res) => {
    let query = supabase.from("Publications").select("id, User!inner(pseudo),Type!inner(type), description, date, Statut_Publication!inner(statut), titre, prix, photos, Couleur!inner(couleur), Matière!inner(matière), État_Meuble!inner(état), Dimensions!inner(hauteur,largeur,longueur), Pièce!inner(pièce)");
...
const updateStatut = async (req, res) => {
    const { data, error } = await supabase.from("Publications").update({ satut_id: 1 }).eq("id", req.params.id).select();
    if (error) {
        console.log(error);
    }
    return data;
...
const deletePost = async (req, res) => {
    const { data, error } = await supabase.from("Publications").delete("*").eq("id", req.params.id);
    if (error) throw error;
    return data;
};
...
const deletePanier = async (req, res) => {
    const { data, error } = await supabase.from("Panier").delete("*").eq("id", req.params.id);
    if (error) throw error;
    return data;
};
